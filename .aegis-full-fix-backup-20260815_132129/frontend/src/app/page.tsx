    ...
    if (minDiff < 0.5) {
      ...
      if (activeThreats && Array.isArray(activeThreats)) {
        activeThreats.forEach((threat: any) => {
          ...
          const socketReady = threatSocket.current?.readyState === WebSocket.OPEN;
          const confidenceOK = Number(threat.confidence ?? 0) >= 0.45;
          const cooldownOK = triggerKey !== lastThreatTrigger.current || now - lastThreatTriggerTime.current >= 5000;
          const shouldTrigger = socketReady && confidenceOK && cooldownOK && !threatRequestInFlight.current;
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (Array.isArray(data.evidence) && data.evidence.length > 0) {
            ...
            setIntelData(intel);
            setAiSummary(`Threat detected. Evidence: ${intel.id}. ${data.assessment?.recommended_action || ""}`);
            if (typeof intel.lat === "number" && typeof intel.lng === "number") {
